        ...
        <section className="space-y-6 lg:col-span-2">
                  ...
                  <li key={msg.id}>
                    <DashboardListRow>
                      <div className="flex items-start justify-between gap-2">
                        ...
                        {!msg.read && (
                          <span className="shrink-0 rounded-full bg-[var(--primary)] px-2 py-0.5 text-[10px] font-medium text-white">
                            New
                          </span>
                        )}
                      </div>
                      <p className="mt-0.5 text-xs text-white/45">{msg.senderName}</p>
          ...
          >
            {data.upcomingInvoices.length === 0 ? (
              <p className="text-sm text-white/40">No invoices on file yet.</p>
            ) : (
              <ul className="space-y-2">
                {data.upcomingInvoices.map((inv) => (
                  <li key={inv.id}>
                    <DashboardListRow className="flex items-center justify-between gap-3 py-2.5">
                      <div className="min-w-0">
                        <p className="truncate text-sm text-white">{inv.invoice_number}</p>
