...
export function InvoicePrintView({ invoice }: { invoice: PrintInvoice }) {
  ...
  return (
    <div className="portal-print-overlay fixed inset-0 z-50 overflow-auto bg-[var(--admin-bg)]">
      <div className="no-print border-b border-[var(--admin-border-subtle)] bg-[color-mix(in_srgb,var(--admin-bg)_92%,transparent)] px-4 py-3 backdrop-blur-xl sm:px-6">
        <div className="mx-auto flex max-w-3xl flex-wrap items-center gap-3">
          <Link href="/portal/invoices" className="admin-btn-ghost px-3 py-2 text-sm">
            ← Back to invoices
          </Link>
          <button type="button" onClick={() => window.print()} className="admin-btn-gold px-4 py-2 text-sm">
            Print / Save as PDF
          </button>
        </div>
      </div>

      <div className="invoice-print mx-auto max-w-3xl bg-white p-8 text-[#0f1a17] print:max-w-none print:p-0">
        <style>{`
          @media print {
            body { background: white !important; }
            .no-print { display: none !important; }
            .portal-print-overlay { position: static; background: white !important; }
          }
        `}</style>

        <header className="border-b border-[#0f1a17]/10 pb-6">
          <p className="text-xs font-semibold uppercase tracking-widest text-[#6f8f72]">DeanVerse Digital</p>
        ...
        {lineItems.length > 0 && (
          <section className="mt-10">
            <h2 className="text-sm font-semibold uppercase tracking-wider text-[#0f1a17]/45">Line items</h2>
              ...
              <tbody>
                {lineItems.map((item, index) => (
                  <tr key={index} className="border-b border-[#0f1a17]/5">
                    ...
                    <td className="py-2 text-right">${Number(item.amount ?? 0).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section className="mt-10 border-t border-[#0f1a17]/10 pt-6">
          <div className="flex items-end justify-between">
            ...
            <p className="text-2xl font-semibold">${Number(invoice.amount).toLocaleString()}</p>
          </div>
        </section>
