...
export function Header() {
  ...
  useEffect(() => {
    ...
    function handleScroll(event: Event) {
      ...
      if (target instanceof Node && mobileNavRef.current?.contains(target)) {
        return;
      }
      closeMobileNav();
    }
    ...
    mobileOpen && typeof document !== "undefined"
      ? createPortal(
          <div
            ref={mobileNavRef}
            id="site-mobile-nav"
            className="site-mobile-nav fixed inset-0 z-40 lg:hidden"
            aria-hidden={false}
          >
            <button
              type="button"
              className="site-mobile-nav-backdrop absolute inset-0 bg-[#0a1210]/90 opacity-100 backdrop-blur-md transition-opacity duration-300"
              aria-label="Close menu"
              onClick={closeMobileNav}
            />
            <div className="site-mobile-nav-panel absolute inset-x-0 bottom-0 top-14 flex max-h-[calc(100dvh-3.5rem)] translate-y-0 flex-col transition-transform duration-300 ease-out sm:top-16">
              <nav
              ...
              <div className="site-mobile-nav-footer shrink-0 border-t border-white/10 px-4 py-4">
                ...
                <Button href="/contact" variant="primary" className="min-h-[48px] w-full">
                  Start a Project
                </Button>
              </div>
            </div>
