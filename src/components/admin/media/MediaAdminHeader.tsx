...
export function MediaAdminHeader({
  ...
  return (
    <header className="admin-content-header sticky top-0 z-20 shrink-0 border-b border-[var(--admin-border-subtle)] bg-[color-mix(in_srgb,var(--admin-bg)_90%,transparent)] px-6 backdrop-blur-xl lg:px-8">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm text-[var(--admin-text-muted)]">
            Upload, organize, and manage images, videos, and documents for your site.
          </p>
        </div>
      ...
      <div className="admin-media-tabs mt-4">
        {MEDIA_TABS.map((tab) => (
          <button
            key={tab.id}
            type="button"
            onClick={() => onTabChange(tab.id)}
            className={cn("admin-media-tab", activeTab === tab.id && "admin-media-tab-active")}
          >
            {tab.label}
            <span className="admin-media-tab-badge">{tabCounts[tab.id]}</span>
          </button>
        ))}
      </div>
    </header>
