...
export function LeadsTable({
  ...
  const menuPortal =
    ...
    createPortal(
      <div
        ref={menuRef}
        className="admin-leads-menu admin-leads-menu-floating"
        role="menu"
      ...
      >
        ...
        {LEAD_STATUSES.map((status) => (
          <button
            ...
            type="button"
            onClick={() => {
              onStatusChange(menuState.lead, status);
              setMenuState(null);
            }}
            className={cn(menuState.lead.status === status && "text-[var(--admin-gold-light)]")}
          >
            Mark as {statusStyle(status).label}
          </button>
        ))}
        {menuState.lead.status !== "converted" && (
          <button
            type="button"
            onClick={() => {
              onConvert(menuState.lead);
              setMenuState(null);
            }}
          >
            Convert to client
        ...
        <a href={`mailto:${menuState.lead.email}`} className="admin-leads-menu-link">
          Send email
        </a>
        <button
          type="button"
          ...
          {leads.map((lead) => (
            <article key={lead.id} className="admin-leads-card">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {lead.source ? <LeadBadge label={lead.source} style={sourceStyle(lead.source)} /> : null}
              </div>
              <p className="mt-2 text-xs text-[var(--admin-text-muted)]">{serviceLabel(lead)}</p>
              <div className="mt-4 flex flex-wrap gap-2">
                <button type="button" onClick={() => onView(lead)} className="admin-leads-action-btn text-xs px-2.5">
                  View
                </button>
                <button type="button" onClick={() => onNotes(lead)} className="admin-leads-action-btn text-xs px-2.5">
                  Notes
                </button>
                {lead.status !== "converted" && (
                  <button type="button" onClick={() => onConvert(lead)} className="admin-leads-action-btn text-xs px-2.5">
                    Convert
                  </button>
                )}
              </div>
  ...
  return (
    <>
      <div className="admin-leads-table-wrap">
        <table className="admin-leads-table w-full min-w-[760px] text-sm">
          <thead>
            ...
            {leads.map((lead) => {
              ...
              return (
                <tr key={lead.id} className="admin-leads-table-row">
                  ...
                  <td className="px-3 py-3">
                    <p className="max-w-[12rem] truncate text-[var(--admin-text)]">{lead.email}</p>
                  ...
                  <td className="max-w-[10rem] px-3 py-3 text-xs leading-relaxed text-[var(--admin-text-muted)]">
                    <span className="line-clamp-2">{serviceLabel(lead)}</span>
                  </td>
                  <td className="px-3 py-3">
                    <LeadStatusBadge status={lead.status} />
                  </td>
                    ...
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          ...
                          setMenuState(
                            menuState?.lead.id === lead.id
                              ? null
                              : { lead, top: position.top, left: position.left },
                          );
                        }}
                        className={cn(
                          "admin-leads-action-btn",
                          menuState?.lead.id === lead.id && "bg-[var(--admin-gold-soft)] text-[var(--admin-gold-light)]",
                        )}
                        aria-label="More actions"
                        aria-expanded={menuState?.lead.id === lead.id}
                      >
